    ...
    def stop_script(self, args):
        ...
        for value in args['selected_list']:
            ...
            if not thread.isRunning():
                log.info(f"{value['index']}号模拟器脚本已停止")
                continue
            thread.fun_object.status = "stop"
            log.info(f"{value['index']}号模拟器脚本停止请求发送")
        return True
...
class TableWidget():
    ...
    def refresh_table(self,dll):
        ...
        hwnd_list = dll.EnumWindow(0,"Chrome","",1+4).split(",")
        hwnd_list.reverse()
        # 添加表格信息
...
class mainwindow(QMainWindow):
    ...
    def password_verfity(self):
        ...
        try:
            # dev_chuanqi
            my_password = Other.get_netpad_text("85b5aa98ae158185")["note_content"].replace("\n","")
        except:
            log.info("遇到错误~")
        if "123" != my_password:
            sys.exit(0)
    ...
    def init_connect(self):
        ...
        self.ui.btn_show_window.clicked.connect(
            lambda: self.task_manage({
                "name": self.fc_run.set_chrome_location,
                "args": {
                    "selected_list": self.table.get_selected_list(),
                    "status": "显示"
                }
            }))
        self.ui.btn_hide_window.clicked.connect(
            lambda: self.task_manage({
                "name": self.fc_run.set_chrome_location,
                "args": {
                    "selected_list": self.table.get_selected_list(),
    ...
    def init_ui(self):
        ...
        # 设置窗口标题
        self.ui.setWindowTitle(f"财神传奇辅助 {self.version}")
        # 窗口置顶
