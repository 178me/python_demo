...
class ZhiHu:
    ...
    def get_session(self):
        url = f"https://xueqiu.com/provider/session/token.json?api_path=%2Fstatuses%2Fupdate.json&_=={time()}"
        ret = requests.get(url, headers=self.headers)
        return ret.json()['session_token']

    def answer(self,comment):
        try:
            #  url = 'https://xueqiu.com/statuses/reply.json'
            url = 'https://xueqiu.com/statuses/update.json'
            answer_data = {
                "status": f"<p>{comment}</p>",
                "session_token": self.get_session(),
            }
            resp = requests.post(url=url, headers=self.headers,
                               params=answer_data)
            rjson = resp.json()
            if not rjson.get('created_at',None):
                raise Exception(rjson["error_description"])
            return rjson.get('created_at')
        except Exception as e:
            logging.exception(e)
        ...
        try:
            ...
            try:
                ...
                if len(answer_invertal) > 1:
                    start_invertal, end_invertal = int(
                        answer_invertal[0]), int(answer_invertal[1])
                print("执行脚本")
                for data in self.parse_data('data.xls'):
                    if self.answer(data['发布内容']):
                        print("发布完毕")
                    else:
                        print("发布失败")
                    sleep(randint(start_invertal, end_invertal))
            except Exception as e:
                print("脚本出现以下错误")
                logging.exception(e)
                input("尝试重新运行或者联系作者~")
