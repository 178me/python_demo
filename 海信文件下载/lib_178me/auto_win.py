...
class GuiAutoMation:
    def __init__(self):
        ...
        log.debug(f"当前插件版本:{self.dll.Ver()}")
        ...
        if not os.path.exists(os.path.join("./", "src")):
            os.mkdir("./src")
        self.dll.SetPath(os.path.join("./", "src"))
    ...
    def create_dm(cls):
        try:
            ...
            dll_path = os.path.join(os.getcwd(), "input", "dm.dll")
    ...
    def save_errpage(self, name=0):
        self.dll.Capture(0, 0, 2000, 2000, f"not_found-{name}.bmp")
    ...
    def bind_window(self, display="gdi", mouse="windows3", keypad="windows", mode=0):
        ...
        if self.dll.BindWindow(self.hwnd, display, mouse, keypad, mode) != 1:
            return False
